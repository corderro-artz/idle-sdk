...
public class QuestProgressTests
{
    ...
    {
        Assert.Throws<ArgumentNullException>(() => new QuestProgress(null!));
    }

    [Fact]
    public void AddProgress_Rejects_NonPositive()
    {
        var progress = new QuestProgress(new QuestDefinition("q", "Quest", "Desc", 3));
    ...
    {
        var progress = new QuestProgress(new QuestDefinition("q", "Quest", "Desc", 2));
